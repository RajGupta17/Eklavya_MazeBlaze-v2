...
esp_err_t rotary_encoder_init(rotary_encoder_info_t * info, gpio_num_t pin_a, gpio_num_t pin_b, gpio_num_t pin_c, gpio_num_t pin_d)
{
    ...
    if (info)
    {
        ...
        gpio_pad_select_gpio(info->pin_c);
        gpio_set_pull_mode(info->pin_c, GPIO_PULLUP_ONLY);
        gpio_set_direction(info->pin_c, GPIO_MODE_INPUT);
        gpio_set_intr_type(info->pin_c, GPIO_INTR_ANYEDGE);

        gpio_pad_select_gpio(info->pin_d);
        gpio_set_pull_mode(info->pin_d, GPIO_PULLUP_ONLY);
        gpio_set_direction(info->pin_d, GPIO_MODE_INPUT);
        gpio_set_intr_type(info->pin_d, GPIO_INTR_ANYEDGE);

        // install interrupt handlers
        gpio_isr_handler_add(info->pin_a, _isr_rotenc, info);
        gpio_isr_handler_add(info->pin_b, _isr_rotenc, info);
        gpio_isr_handler_add(info->pin_c, _isr_rotenc, info);
        gpio_isr_handler_add(info->pin_d, _isr_rotenc, info);
...
esp_err_t rotary_encoder_flip_direction(rotary_encoder_info_t * info)
{
    ...
    if (info)
    {
        ...
        info->pin_b = info->pin_c;
        info->pin_c = info->pin_d;
        info->pin_d = temp;
...
esp_err_t rotary_encoder_uninit(rotary_encoder_info_t * info)
{
    ...
    if (info)
    {
        ...
        gpio_isr_handler_remove(info->pin_c);
        gpio_isr_handler_remove(info->pin_d);
    }
    else
    {
        ESP_LOGE(TAG, "info is NULL");
